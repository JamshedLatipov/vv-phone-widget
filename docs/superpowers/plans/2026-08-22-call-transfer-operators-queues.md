...
**Обязательное требование этой задачи, важнее всего остального в ней.** Сегодня на main `POST /api/calls/transfer` принимает `target` как сырую строку с единственной проверкой на непустоту, а `endpointFor` возвращает её нетронутой, если она содержит `/`. Значит оператор с правом `calls:create` может задать `Local/<что угодно>@org-<чужая-организация>-queues` — выбрать чужой диалплан-контекст и запятыми дописать позиционные аргументы `Queue()` (среди них Gosub и AGI). Двигать он может только свой канал: `assertChannelBelongsToTenant` это удерживает. Дыра предсуществующая, не внесена этой работой.

`assertTargetAllowed` её закрывает: `PBX_SAFE` не пропускает `/`, поэтому любая полностью-квалифицированная цель отвергается до входа в сервис. Контроллер — единственный вызыватель `blindTransfer`/`attendedTransfer`, так что гейт на нём закрывает путь целиком.

Проверить это отдельным тестом, а не считать следствием: запрос с `target: "Local/x@org-other-queues"` должен получить отказ, и `blindTransfer` не должен быть вызван вообще.
...
> ## ПЕРЕСМОТР: пикер переезжает в отдельное окно
>
> Задачи 12–14 ниже описывают встроенную панель внутри экрана звонка. **Так не получится**, и это выяснилось замером, а не на глаз.
>
> Окно панели — 496 px: 600 из `ShellGeometry.PanelHeight` минус 58 верхней панели минус 46 нижней навигации. Содержимое экрана звонка при открытой панели перевода и **нуле** строк в списке — 508–525 px. То есть кнопка отбоя уходит под сгиб ещё до загрузки первого оператора. Скрытие ряда быстрых действий (~90 px) закрывает большую часть разрыва, но не весь, а `MaxHeight` не помогает вовсе: лимит ограничивает вклад списка только когда список непустой, при нуле строк `ScrollViewer` и так занимает ноль. Дефицит заложен в раскладку.
>
> **Новая форма — своё окно, по образцу уже существующих диалогов** (`ScriptsDialog`, `TaskDialog`, `SmsComposeDialog`), у каждого из которых есть лаунчер и `SingleWindowGuard`.
>
> Ключевое требование, унаследованное от `ScriptsWindowLauncher`: окно открывается через `window.Show(owner)`, **никогда** через `ShowDialog`. Модальный диалог отключает владельца, и пока он открыт, оператор не может ни положить трубку, ни ответить на следующий звонок. Комментарий в `ScriptsWindowLauncher` описывает ровно этот инцидент — повторять его нельзя.
>
> Состав работ вместо старых 12–14:
>
> | Что | Где |
> |---|---|
> | `TransferDialog.axaml` | окно пикера: вкладки, поиск, статус, повтор, список, ручной ввод |
> | `TransferDialog.axaml.cs` | загрузка, состояния, строки, событие `TransferSelected` с `TransferRequest` |
> | `TransferWindowLauncher.cs` | `Show(owner)`, guard, `CloseIfOpen()` при истечении сессии |
> | `App.TransferWindows` | ещё один `SingleWindowGuard` |
> | `ActiveCallView` | кнопка перевода открывает окно; встроенная `TransferPanel` удаляется вместе с ручным вводом, который переезжает в окно |
> | `MainWindow` | маршрутизация выбранной цели: бэк, при `ChannelUnresolved` — SIP REFER для оператора |
>
> Логика из задачи 13 переносится в код-бихайнд окна почти без изменений — меняется хозяин, а не содержание.
...
**Files:** нет — проверка. Единственная задача плана, которую нельзя закрыть автоматически: нужен живой звонок, реальная АТС и второй оператор.

**Предусловие.** Слайсы 1–3 бэкенда задеплоены, миграция `2026082200000-AddQueueTransferContext` применена. Проверить, что контекст очередей появился:

```
psql "$DATABASE_URL" -c "SELECT context, exten, priority, app, appdata FROM extensions WHERE context LIKE 'org-%-queues' ORDER BY context, exten, priority"
```

Ожидание: на каждую организацию три строки — `h/1/NoOp`, `_./1/Queue ${EXTEN},n`, `_./2/Hangup`. Пусто означает, что миграция не выполнилась: реестр миграций в этом репозитории собирается вручную, проверить запись в `apps/back/src/app/modules/calls/migrations/index.ts`.

- [ ] **Шаг 1. Список целей отвечает**

```
curl -s -H "Authorization: Bearer $TOKEN" $BACKEND/api/calls/transfer-targets
```

Ожидание: `{"operators":[…],"queues":[…]}`. Токен без права `calls:create` → 403. Оператора, который сейчас на звонке, на перерыве, на паузе супервайзера или не зарегистрирован, в списке быть не должно; себя — тоже.

- [ ] **Шаг 2. Запустить виджет**

Run: `dotnet run --project OrbitalSIP/OrbitalSIP.csproj`

- [ ] **Шаг 3. Принять звонок, открыть окно перевода**

Ожидание: открывается **отдельное окно** 420×640, вкладка «Операторы» активна, список заполнен. Проверить главное, ради чего окно и появилось: **экран звонка позади остаётся полностью рабочим** — кнопка отбоя видна и нажимается, пока окно перевода открыто. Нажать её и убедиться, что звонок завершается.

- [ ] **Шаг 4. Перевод на оператора**

Принять звонок заново, открыть окно, выбрать коллегу.
Ожидание: коллеге приходит звонок, у переводящего звонок завершается, тост «Звонок переведён», окно закрывается. В логе виджета — `Transfer requested: Extension -> …`.

Проверить аудит:
```
psql "$DATABASE_URL" -c "SELECT action, resource_id, metadata, actor_name FROM audit_log WHERE action = 'calls.transfer' ORDER BY created_at DESC LIMIT 3"
```
Ожидание: свежая строка с `targetKind: extension`, `type: blind` и именем оператора.

- [ ] **Шаг 5. Перевод в очередь**

Ожидание: вкладка «Очереди» показывает очереди организации; после выбора абонент попадает в очередь.

```
psql "$DATABASE_URL" -c "SELECT time, queuename, event FROM queue_log ORDER BY time DESC LIMIT 5"
```

**Важно и заранее известно:** если у очереди `leavewhenempty = true` (это дефолт и колонки, и формы создания) и в ней нет залогиненных участников либо все на паузе — Asterisk выбросит абонента, `Queue()` завершится, и следующий приоритет `Hangup` **разъединит его**. Проверять на очереди, где кто-то залогинен. Это известное ограничение фазы 1, а не дефект.

- [ ] **Шаг 6. Деградация при недоступном бэкенде**

Погасить бэк, принять звонок, открыть окно.
Ожидание: списка нет, вкладки скрыты, остаётся поле ручного ввода. Ввести номер коллеги — перевод проходит через локальный SIP REFER. Это единственный путь, где фолбэк допустим.

- [ ] **Шаг 7. Отказ по чужой цели**

```
curl -s -X POST -H "Authorization: Bearer $OTHER_ORG_TOKEN" -H 'Content-Type: application/json'   -d '{"channelId":"<живой канал>","target":"sales","targetKind":"queue"}' $BACKEND/api/calls/transfer
```
Ожидание: `{"ok":false,"error":"Unknown queue \"sales\""}`.

- [ ] **Шаг 8. Регрессия безопасности**

```
curl -s -X POST -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json'   -d '{"channelId":"<живой канал>","target":"Local/1042@org-<чужой-uuid>-queues"}' $BACKEND/api/calls/transfer
```
Ожидание: отказ, звонок **не** перемещён. Это та дыра, что закрыта двумя слоями — фильтром символов в `endpointFor` и проверкой принадлежности в контроллере. Если запрос проходит — остановиться и не выкатывать.

- [ ] **Шаг 9. Разбор расхождений**

Любое расхождение на шагах 3–8 чинить до мержа слайса 5. Шаг 8 — блокирующий.
