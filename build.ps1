...
$msi = "$root\dist\proffi-phone-$newVersion.msi"
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "Done! dist\proffi-phone-$newVersion.exe" -ForegroundColor Green
    ...
    # Installer name comes from installer/OrbitalSIP.iss OutputBaseFilename = proffi-phone-<ver>.
    $installer = "$root\dist\proffi-phone-$newVersion.exe"
    $tag       = "v$newVersion"
